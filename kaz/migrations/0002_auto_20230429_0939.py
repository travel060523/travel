...
def beginning(apps, schema_editor):
    ...
    claim.finish  = datetime.now() + timedelta(days=37);
    claim.details = ''
    claim.result =  ''
    claim.save()
    ...
    claim.user_id = 4
    claim.hotel = hotel
    claim.start  = datetime.now() + timedelta(days=45);
    claim.finish  = datetime.now() + timedelta(days=52);
    claim.details = ''
    claim.result =  ''
    claim.save()
    ...
    claim.user_id = 5
    claim.hotel = hotel
    claim.start  = datetime.now() + timedelta(days=51);
    claim.finish  = datetime.now() + timedelta(days=58);
    claim.details = ''
    claim.result =  ''
    claim.save()
    ...
    claim.user_id = 6
    claim.hotel = hotel
    claim.start  = datetime.now() + timedelta(days=41);
    claim.finish  = datetime.now() + timedelta(days=48);
    claim.details = ''
    claim.result =  ''
    claim.save()
    ...
    claim.user_id = 7
    claim.hotel = hotel
    claim.start  = datetime.now() + timedelta(days=60);
    claim.finish  = datetime.now() + timedelta(days=67);
    claim.details = ''
    claim.result =  ''
    claim.save()

    reviews = Reviews()
    reviews.dater = datetime.now() - timedelta(days=35);
    reviews.hotel = hotel
    reviews.user = claim.user
    reviews.rating = 5
    ...
    claim.user_id = 8
    claim.hotel = hotel
    claim.start  = datetime.now() + timedelta(days=65);
    claim.finish  = datetime.now() + timedelta(days=72);
    claim.details = ''
    claim.result =  ''
    claim.save()
    ...
    claim.user_id = 9
    claim.hotel = hotel
    claim.start  = datetime.now() + timedelta(days=31);
    claim.finish  = datetime.now() + timedelta(days=38);
    claim.details = ''
    claim.result =  ''
    claim.save()
    ...
    news.details =  """Казахстанский блогер, путешественник и общественный деятель Александр Цой посетил 95 стран: до пандемии мужчина успевал объехать по 10-12 стран в год, а в период "ковидных" ограничений побывал в крупнейших городах Казахстана. В беседе с корреспондентом Tengritravel.kz путешественник рассказал о своих наработках и опыте.<br>
    Александр признался, что изучает место следующего выезда, у него есть свои лайфхаки.<br>
    "Я заранее стараюсь читать об этих местах, даже если у нас будет гид. Захожу в Instagram, ввожу эту достопримечательность и получаю 10 лучших фото, на фоне Тадж-Махала например.<br>
    ...
    news.details = """Лондон, Париж и Амстердам возглавили рейтинг городов Европы, подходящих для туризма и долгосрочного проживания. Консалтинговая компания Resonance Consultancy расставила по местам 100 лучших мегаполисов континента, передает Tengritravel.kz.<br>
    По информации ресурса Time Out, рейтингу можно доверять, ведь мнение составителей авторитетно.<br>
    "Resonance Consultancy, фирма, организовавшая WorldsBestCities.com, составила рейтинг 100 лучших городов Европы на 2023 год. При составлении списка учитывались многие факторы, в том числе пешеходная доступность, достопримечательности, разнообразие, впечатления, уровень занятости и даже хештеги в Instagram", - пишет источник.<br>
    Топ-10 лучших европейских городов выглядит так:<br>
    ...
    news.details = """ританский путешественник приобрел дешевый билет на рейс и оказался в сказочных условиях: мужчина был единственным пассажиром на борту и бортпроводники относились к нему, "как к королю", передает Tengritravel.kz со ссылкой на Daily Star.<br>
    По информации издания, 65-летний Пол Уилкинсон летел из португальского Фару в Белфаст самолетом авиакомпании Jet2.<br>
    "Этот полет мужчина не забудет никогда: экипаж обращался с ним, как с "членом королевской семьи". Пол получил VIP-обслуживание на борту, "как будто летел на собственном частном самолете". Пассажир был потрясен, когда, подойдя к выходу на посадку, обнаружил, что он единственный в очереди в самолет.<br>
    Он даже спросил сотрудников аэропорта, был ли его рейс задержан или отменен, но они сообщили ему, что самолет собирается взлететь, так как Пол - единственный пассажир, забронировавший билет на рейс", - рассказывает источник.<br>
    """
    ...
    news.details = """Актерское мастерство гида в Индонезии покорило социальные сети: мужчина нашел неожиданную "подработку" - он стал популярным после подбора для своих клиентов наилучших вариантов для фото на фоне водопада, передает Tengritravel.kz.<br>
    По информации автора ролика с гидом, эпизод снят у водопада Канто Лампо на Бали.<br>
    ...
    news.details = """Авиамаршрут Шымкент - Дели - Шымкент впервые запускают из Казахстана, передает Tengritravel.kz со ссылкой на пресс-службу Комитета гражданской авиации.<br>
    По информации ведомства, маршрут будет запущен в конце мая - планируются два рейса в неделю.<br>
    "Впервые в истории открывается новый авиамаршрут Шымкент - Дели - Шымкент. Рейсы будут выполняться авиаперевозчиком Fly Arystan c 22 мая 2023 года с частотой 2 раза в неделю по понедельникам и четвергам на воздушном судне типа Airbus-320", - говорится в сообщении.<br>
    Ранее стало известно, что Казахстан планирует ввести безвизовый режим с Китаем, а мы рассказали о малоизвестных туристических казахстанских маршрутах.<br>
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunSQL("""CREATE VIEW view_hotel AS
                        SELECT hotel.id, country.title AS country, region.title AS region, hotel.title, hotel.details, hotel.photo, hotel.price, hotel.flight, (SELECT AVG(rating) FROM reviews WHERE reviews.hotel_id = hotel.id) AS avg_rating 
